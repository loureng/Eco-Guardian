                       ...
                       historyChartData.map((item, i) => {
                         ...
                         return (
                           <div
                             ...
                             tabIndex={0}
                             role="button"
                             aria-label={`${item.days} dias (${isFuture ? 'futuro' : 'passado'}), em ${item.date.getDate()}/${item.date.getMonth()+1}`}
                             className="flex-1 flex flex-col items-center justify-end h-full gap-1 group/bar relative focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 rounded-sm cursor-help"
                           >
                              {/* Floating Tooltip */}
                              <div className="absolute -top-8 opacity-0 group-hover/bar:opacity-100 group-focus/bar:opacity-100 transition-all duration-200 transform translate-y-2 group-hover/bar:translate-y-0 group-focus/bar:translate-y-0 bg-slate-800 text-white text-[10px] font-bold py-1 px-2 rounded shadow-lg z-20 whitespace-nowrap pointer-events-none">
                                {item.days} dias
