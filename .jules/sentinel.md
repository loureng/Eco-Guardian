...
## 2026-02-01 - AI Grounding Output Sanitization
**Vulnerability:** The Chatbot component trusted `chunk.web.uri` and `chunk.maps.uri` from the AI's "grounding chunks" (sources) directly in `href` attributes. This posed an XSS risk if the AI (or a compromised upstream service) returned a `javascript:` URI.
**Learning:** AI structured output is not inherently safe. Even "metadata" fields like URIs must be validated as strictly as user input.
**Prevention:** Applied `isSafeUrl` check to all `href` attributes derived from AI responses in `Chatbot.tsx`.
